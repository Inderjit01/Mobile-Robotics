...
from math import pi, atan2, exp, sqrt, sin, cos, tan
import random
...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        # ADD CODE HERE
        # Compute new pose (x and y coordinates and orientation)
        alpha = random.gauss(0.0, self.steering_noise)
        steering = max(-max_steering_angle, min(max_steering_angle, motion[0] + alpha))
        distance = max(0.0, motion[1] + random.gauss(0.0, self.distance_noise))
        ...
        x = self.x + motion[1] * cos(self.orientation)
        y = self.y + motion[1] * sin(self.orientation)
        theta = (self.orientation + motion[0]) % (2 * pi)
        
        result = robot()
        result.length=self.length
        # ADD CODE HERE
        # Set result robot with new pose.
...
def particle_filter(motions, measurements, N=500): # don't change N!
    # --------
    #
    # Make particles
    # 
